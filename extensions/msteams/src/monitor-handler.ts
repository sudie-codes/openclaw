...
import {
  dispatchMSTeamsPluginInteractiveHandler,
  extractMSTeamsAdaptiveCardInteractiveData,
  ...
  if (originalRun) {
    handler.run = async (context: unknown) => {
      ...
      if (ctx.activity?.type === "invoke" && ctx.activity?.name === "adaptiveCard/action") {
        const interactive = extractMSTeamsAdaptiveCardInteractiveData(ctx.activity?.value);
        if (interactive) {
          const senderId =
            ctx.activity.from?.aadObjectId?.trim() || ctx.activity.from?.id?.trim() || undefined;
          const senderName = ctx.activity.from?.name?.trim() || undefined;
          const conversationId =
            normalizeMSTeamsConversationId(ctx.activity.conversation?.id ?? "") ??
            ctx.activity.conversation?.id?.trim() ??
            "";
          const parentConversationId = ctx.activity.conversation?.id?.trim() || undefined;
          const interactionId =
            ctx.activity.id?.trim() ||
            [conversationId, senderId, interactive.data].filter(Boolean).join(":");
          const dispatched = await dispatchMSTeamsPluginInteractiveHandler({
            ...
            ctx: {
              ...
              ...(ctx.activity.replyToId?.trim()
                ? { threadId: ctx.activity.replyToId.trim() }
                : {}),
              auth: {
              ...
              interaction: {
                ...
                ...(interactive.actionData ? { actionData: interactive.actionData } : {}),
                ...(interactive.decisionHint ? { decisionHint: interactive.decisionHint } : {}),
              },
            },
            turnContext: ctx,
          });
