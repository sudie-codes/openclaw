...
vi.mock("./channel.runtime.js", () => ({
  msTeamsChannelRuntime: {
    ...
    listThreadMSTeams: listThreadMSTeamsMock,
...
describe("msteamsPlugin message actions", () => {
  ...
  it("advertises thread-list in the message tool surface", () => {
    expect(
      msteamsActionsAdapter.describeMessageTool?.({
        cfg: {
          channels: {
    ...
    ).toContain("thread-list");
  });

  it("routes thread-list through listThreadMSTeams with explicit thread target", async () => {
    await expectSuccessfulAction({
      mockFn: listThreadMSTeamsMock,
      mockResult: {
        teamId: "team-1",
        channelId: "channel-1",
        ...
        sourceIds: ["root-1", "reply-1"],
        messages: [{ id: "root-1", senderName: "Alex", text: "hello", source: "root" }],
      },
      action: "thread-list",
      actionParams: {
        teamId: "team-1",
      ...
      details: okMSTeamsActionDetails("thread-list", {
        teamId: "team-1",
        channelId: "channel-1",
        rootMessageId: "root-1",
        truncated: true,
  ...
  it("rejects thread-list without an explicit team/channel/root target", async () => {
    await expectActionParamError(
      "thread-list",
      {
        teamId: "team-1",
      },
      "thread-list requires teamId, channelId, and rootMessageId.",
    );
  });
